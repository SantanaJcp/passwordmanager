...
if [[ "$(uname -s)" != Linux || "$(uname -m)" != x86_64 ]]; then
  echo "ticket 23 laboratory requires Linux x86_64" >&2; exit 1
fi
for tool in /usr/bin/tmux /usr/bin/wl-copy /usr/bin/wl-paste; do test -x "$tool"; done
[[ "$(/usr/bin/wl-copy --version | head -1)" == "wl-clipboard 2.3.0" ]]
...
unshare --user --mount --map-users "0:$host_uid:1" --map-users "1:$subuid:65535" \
  ...
  env PYTHONDONTWRITEBYTECODE=1 python3 "$root/crates/pm-custody/tests/tui_content_lab.py" \
