...
if [[ "$(uname -s)" != Linux || "$(uname -m)" != x86_64 ]]; then
  echo "ticket 24 laboratory requires Linux x86_64" >&2; exit 1
fi
for tool in /usr/bin/tmux; do test -x "$tool"; done
...
unshare --user --mount --map-users "0:$host_uid:1" --map-users "1:$subuid:65535" \
  ...
  env PYTHONDONTWRITEBYTECODE=1 python3 "$root/crates/pm-custody/tests/tui_access_lab.py" \
