...
PREFETCH_CANARY = b"synthetic-ticket28-kernel-queued-canary" * 8
...
def start_create_socket(cli, vault):
    ...
    try:
        ...
        writer.close()
        raise
    return process, writer, child_input
...
def main():
    ...
    try:
        ...
        subject, writer, input_probe = start_create_socket(cli, root / "guarded.sqlite3")
        processes.append(subject)
        endpoints.extend((writer, input_probe))
        ...
        writer.sendall(PASSWORD + b"\n" + PASSWORD + b"\n" + PREFETCH_CANARY)
        wait_line(subject, b"Confirm master password:\n")
        remaining = queued_bytes(input_probe)
        assert remaining >= len(PREFETCH_CANARY), (
            "stdin implementation prefetched the queued canary into process memory",
            remaining,
        )
        recovery = wait_line(
            subject,
        ...
        assert PASSWORD not in stdout + stderr and PREFETCH_CANARY not in stdout + stderr
    ...
    print(
        ...
        "locked-secrets=1 memlock-denial=closed stdin=native-unbuffered "
        "argv+env+stdout+stderr=clean cleanup=verified"
    )
